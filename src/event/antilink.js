...
export const handleAntilink = async (m, sock, logger, isBotAdmins, isAdmins, isCreator) => {
    ...
    if (cmd === 'antilink') {
        ...
        if (!m.isGroup) {
            await sock.sendMessage(m.from, { text: 'OH no not in dm! use this in groups' }, { quoted: m });
            return;
        }

        if (!isBotAdmins) {
            await sock.sendMessage(m.from, { text: 'Mkae samsung admin to activate antilink' }, { quoted: m });
            return;
        }
        ...
        if (action === 'on') {
            if (isAdmins) {
                ...
                await sock.sendMessage(m.from, { text: 'Antilink enabled' }, { quoted: m });
            } else {
                await sock.sendMessage(m.from, { text: 'Only admins can enable the antilink.' }, { quoted: m });
            }
            return;
        }
        ...
        if (action === 'off') {
            if (isAdmins) {
                ...
                await sock.sendMessage(m.from, { text: 'Antilink disabled .' }, { quoted: m });
            } else {
                await sock.sendMessage(m.from, { text: 'Only admins can disable the antilink ' }, { quoted: m });
            }
            return;
        }
    ...
    if (antilinkSettings[m.from]) {
        if (m.body.match(/(chat.whatsapp.com\/)/gi)) {
            if (!isBotAdmins) {
                await sock.sendMessage(m.from, { text: `samsung needs to be an admin to remove links.` });
                return;
            }
